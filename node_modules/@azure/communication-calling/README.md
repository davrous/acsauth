...
interface CaptionsInfo = {
    ...
    sequenceId: number, // The sequence number of the caption - used to determine ordering.
    timestamp: Date, // Timestamp of when the captioned words were initially spoken.
    durationMs: number, // The duration of the spoken words in milliseconds.
    text: string, // The caption text.
    isFinal: boolean, // Whether or not this caption data is in an intermediate state. 
...
Notice how in the last packet, the `isFinal` field is true.
...
Local participant ID is an extended feature of the core `Call` API to get unique identifier ID for the current participant in the call in a GUID format:

```js
const localParticipantId = call.features(Feature.DebugInfo).localParticipantId;
```
